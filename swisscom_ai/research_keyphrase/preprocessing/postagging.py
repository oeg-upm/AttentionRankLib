...
class PosTaggingSpacy(PosTagging):
    ...
    def pos_tag_raw_text(self, text, as_tuple_list=True):
        ...
        doc = self.nlp(text)
        if as_tuple_list:
            return [[(token.stopwords, token.tag_) for token in sent] for sent in doc.sents]
        return '[ENDSENT]'.join(' '.join(self.separator.join([token.stopwords, token.tag_]) for token in sent) for sent in doc.sents)
